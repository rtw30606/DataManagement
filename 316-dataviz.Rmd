...
with the grammar of graphics and ggplot2 (an R extension for graphics).
...
A grammar has been designed by Wilkinson[^dataviz-1] for creating
...
Vega[^dataviz-2] is a recent formulation building on Wilkinson's work.
...
message. ggplot2 is a descendant of ggplot2 and adds new features to
support interactive visualization using shiny, another R package, which
we will cover later.
...
ggplot2 uses the pipe function to specify a linear sequence of
processing.

-   The foundation is the ggplot2 function, which identifies the source
    of the data and what is to be plotted. In the following example, the
...
counts a column graph is created.
...
site]{.ul}](http://ggplot2.tidyverse.org). Another useful web site for
data visualization is [[FlowingData]{.ul}](http://flowingdata.com),
...
11. Read the table
    ...
    color chart[^dataviz-6] for a choice of colors.
...
[^dataviz-5]: Note these prices have been transformed from the original
