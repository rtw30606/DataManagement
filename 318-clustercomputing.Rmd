...
![](Figures/Chapter 18/bottleneck.png)

The people skills problem is being addressed by the many universities
that have added graduate courses in data analytics. The **Lambda Architecture**[^clustercomputing-1] 
is a proposed general solution to the speed and cost problem.

[^clustercomputing-1] : Marz, N., & Warren, J. (2012). *Big Data*: Manning Publications.
...
![](Figures/Chapter 18/merge-speed-serving.png)
...
![](Figures/Chapter 18/batch-and-speed.png)
...
Hadoop, an Apache project,[^clustercomputing-2] supports distributed processing of large
...
[^clustercomputing-2] : [[http://hadoop.apache.org]{.underline}](http://hadoop.apache.org)

A market analysis projects that the Hadoop market is growing at 58% per
year.[^clustercomputing-3] It also asserts that, "Hadoop is the only cost-sensible and
...
[^clustercomputing-3] : [[https://www.marketanalysis.com/?p=279]{.underline}](https://www.marketanalysis.com/?p=279) (Hadoop Market Forecast 2017-2022, May 2017)
...
![](Figures/Chapter 18/splitting-cluster.png)
...
Spark is an Apache project[^clustercomputing-4] for cluster computing that was initiated
...
[^clustercomputing-4] : [[https://spark.apache.org]{.underline}](https://spark.apache.org)
...
case, we will use the sparklyr,[^clustercomputing-5] an R interface to Spark. This
...
[^clustercomputing-5] : [[http://spark.rstudio.com]{.underline}](http://spark.rstudio.com)

Before starting with sparklyr, you need to check that you have latest
version of Java on your machine.[^clustercomputing-6] Use RStudio to install sparklyr.
For developing and testing on your computer, install a local version of
Spark.

[^clustercomputing-6] : [[https://www.java.com/en/download/help/version\_manual.xml]{.underline}](https://www.java.com/en/download/help/version_manual.xml)
...
Use the `spark_connect` function to connect to Spark either locally or on
...
York's Central Park[^clustercomputing-7] and we want to determine how often each
particular temperature occurred. R

[^clustercomputing-7] : [[http://www.erh.noaa.gov/okx/climate/records/monthannualtemp.html]{.underline}](http://www.erh.noaa.gov/okx/climate/records/monthannualtemp.html)
...
systems. The IEEE Standard for Floating-Point Arithmetic (IEEE 754)[^clustercomputing-8]
...
[^clustercomputing-8] : [[https://en.wikipedia.org/wiki/IEEE\_floating\_point]{.underline}](https://en.wikipedia.org/wiki/IEEE_floating_point)

> ❓*Skill builder* Aedo the tabulation example with temperatures in Celsius.
...
> ❓*Skill builder* A file[^clustercomputing-9] of electricity costs for a major city contains a timestamp and cost separated by a comma. Compute the minimum, mean, and maximum costs.     

[^clustercomputing-9] : [^[http://www.richardtwatson.com/data/electricityprices.csv]{.underline}^](http://people.terry.uga.edu/rwatson/data/electricityprices2010_14.csv)
...
1.  Write Spark code for the following situations.
    ...
    e. Read the national GDP file,[^clustercomputing-10] which records GDP in millions,
        and count how many countries have a GDP greater than or less
        than 10,000 million.

[^clustercomputing-10] : [[http://www.richardtwatson.com/data/GDP.csv]{.underline}](http://people.terry.uga.edu/rwatson/data/GDP.csv) 