...
[^IntroR-1]: RStudio provides a keyboard command, which is Alt+- (Windows &
    Linux) or Option+- (Mac)
...
The dplyr package[^IntroR-3] provides functions for efficiently manipulating
data sets. By providing a series of basic data handling functions and
use of the pipe function ( %\>% ),[^IntroR-4] dplyr implements a grammar for
...
[^IntroR-4]: Short cuts is Ctrl+Shift+M (Windows & Linux) and Cmd+Shift+M (Mac)
...
dplyr.::inner_join().[^IntroR-5] This is the same as joining two tables using a
...
[^IntroR-5]: This is the R notation for indicating the package (dplyr) to which
...
[^IntroR-7]: You will likely get a warning message of the form "unrecognized
...
more books,[^IntroR-8] both of a general and specialized nature. The [[*R
Reference
...
4.  Merge the data for weather (database weather at richardtwatson.com
    ...
    **Hint**: MySQL might return a timestamp with decimal seconds (e.g.,
    ...
    characters using substr(),[^IntroR-11] so that the two timestamp columns
...
[^IntroR-10]: Note these prices have been transformed from the original values,
