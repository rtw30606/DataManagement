...
| markets and jurisdictions. The need for such a standard was        |
| amplified by Lehman Brothers collapse in 2008. Lehman had 209      |
| registered subsidiaries, legal entities, in 21 countries, and it   |
| was party to more than 900,000 derivatives contracts upon its      |
| collapse. Key stakeholders, such as financial regulators and       |
| Lehman's creditors, were unable to assess their exposure.          |
| Furthermore, others were unable to assess the possible ripple on   |
| them of the effects of the collapse because of the transitive      |
| nature of many investments (i.e., A owes B, B owes C, and C owes   |
| D).                                                                |
|                                                                    |
| The adoption of a global legal entity identifier (LEI), should     |
| improve financial system regulation and corporate risk management. |
| Regulators will find it easier to monitor and analyze threats to   |
| financial stability and risk managers will be more able evaluate   |
| their companies' risks.                                            |
|                                                                    |
...
Query Language (SQL).**[^singleentity-1]

[^singleentity-1]: Officially pronounced as "S-Q-L," but often also
...
+------------+------------+------------------------------------------+
|            | FLOAT(p)   | A scientific format number of p binary   |
|            |            | digits precision                         |
+------------+------------+------------------------------------------+
|            | DE CI      | A packed decimal number of p digits      |
|            | MAL(p,q)   | total length; q decimal spaces to the    |
|            |            | right of the decimal point may be        |
|            |            | specified                                |
+------------+------------+------------------------------------------+
| String     | CHAR(n)    | A fixed-length string of n characters    |
+------------+------------+------------------------------------------+
|            | VA         | A variable-length string of up to n      |
|            | RCHAR(n)   | characters                               |
+------------+------------+------------------------------------------+
|            | TEXT       | A variable-length string of up to 65,535 |
|            |            | characters                               |
+------------+------------+------------------------------------------+
| D ate/time | DATE       | Date in the form yyyymmdd                |
+------------+------------+------------------------------------------+
|            | TIME       | Time in the form hhmmss                  |
+------------+------------+------------------------------------------+
|            | T IMESTAMP | A combination of date and time to the    |
|            |            | nearest microsecond                      |
+------------+------------+------------------------------------------+
|            | TIME WITH  | Same as time, with the addition of an    |
|            | TIME ZONE  | offset from universal time coordinated   |
|            |            | (UTC) of the specified time              |
+------------+------------+------------------------------------------+
|            | T IMESTAMP | Same as timestamp, with the addition of  |
|            | WITH TIME  | an offset from UTC of the specified time |
|            | ZONE       |                                          |
+------------+------------+------------------------------------------+
| Logical    | BOOLEAN    | A set of truth values: TRUE, FALSE, or   |
|            |            | UNKNOWN                                  |
+------------+------------+------------------------------------------+
...
| [shrcode]{.ul} | shrfirm             | shrprice |    shrqty | shrdiv | shrpe |
|:---------------|:--------------------|---------:|----------:|-------:|------:|
| FC             | Freedonia Copper    |     27.5 |    10,529 |   1.84 |    16 |
...
rows and 2 columns. These columns are shaded in the table.
...
+-----------------------------------------------------------+
| Warning: The preceding query is often mistakenly written  |
| as                                                        |
|                                                           |
| > SELECT shrfirm, shrpe from share                        |
| >                                                         |
...
| *Skill builder*                              |
|                                              |
| List the firms containing "ian" in their     |
| name.                                        |
+----------------------------------------------+
...
![](Figures/Chapter%203/cd-1.png){width="152"}
...
|                                                                    |
|     d.  What tracks are longer than the average length of tracks   |
|         on the CD?                                                 |
|                                                                    |
...
|                                                                    |
| 2.  Add the 13 tracks from The Manhattan Transfer's Swing CD in    |
|     the following table to the track table:                        |
|                                                                    |
| | Track | Title                    | Length |                      |
| |:------|:-------------------------|:-------|                      |
