...
information about its buildings and parks via their mobile phone in the
language of choice. Their smartphone will determine their location and
then select from a database details of the immediate environment. Data
...
management of time-varying, or **temporal**, data is availed when a
database management system has built-in temporal support. As a result,
there has been extensive research on temporal data models and RDBMSs for
more than a decade. The management of temporal data is another skill
required of today's data management specialist.

The [Open Geospatial Consortium](https://www.ogc.org), Inc. (OGC) is a nonprofit international
...
other geospatial technologies. RDBMS vendors (e.g., MySQL) have
...
form of RDBMS. Geospatial modeling is based on three key concepts: theme,
...
![](Figures/Chapter%2014/political%20unit.png){width="360"}

### SQL/MM Spatial {.unnumbered}

SQL/MM, also known as ISO 13249, is an extension of SQL to handle
spatial data. It uses the prefix ST_ for tables, views, data types, and
...
think of ST_ as meaning *Spatial Type*.

MySQL has data types for storing geometric data, which are:

| Type       | Representation | Description                       |
|:-----------|:----------------|:-----------------------------------|
| Point      | POINT(x y)  | A point in space (e.g., a city’s  |
...
a SRID of 0, Ireland is represented in the following map using a Cartesian SRS. Since Ireland is small relative the size of the earth, this is a reasonable
approximation.
...
![](Figures/Chapter%2013/Ireland%20grid.png)
...
The two sets of values for the column `boundary` define the boundaries
...
`ST_GeomFromText` is an MySQL function to convert text into a geometry
data form.
...
> ❓ **Skill builder**
>
>Create the three tables for the example and insert
...
the area of one grid unit is 1406 km (526 square miles).
...
You can measure the distance between two points using ST_Distance. You
...
You can insert data using `ST_GeomCollFromText`, as the following
example illustrates:
...
4.  The address bar (URL) will change. Examine the full link. For example:
    https://www.google.com/maps/place/Athens-Ben+Epps+Airport+-+AHN/@33.9519038,-83.3273184,17z/data=!3m1!4b1!4m5!3m4!1s0x88f66b676ce4ce85:0x749d5ade6813b384!8m2!3d33.9519038!4d-83.3251297.

5.  The latitude and longitude are contained in the URL following AHN/@.
    In this case, latitude is: 33.9519038 and longitude: --83.3273184.

## R-tree {.unnumbered}

Conventional RDBMSs were developed to handle one-dimensional data
...
figure, the drawn region (it shaded) completely covers object
...
Most of the major RDBMS vendors support management of spatial data.
...
-   **Transaction time** is the timestamp applied by the system when
    ...
    price of a product, one approach might be to update the
    ...
    new price and the time when the insert occurred. In a database, **immmutable data** cannot be (or shouldn’t be) deleted or modified. Most traditional databases store data in a **mutable** format, meaning the database overwrites the older data when new data is available.
...
[^spatial-2]: Adapted from: Goralwalla, I. A., M. T. Özsu, and D.
    Szafron. 1998. *An object-oriented framework for temporal data
    models. In Temporal databases: research and practice*, edited by O.
    Etzion, S. Jajoda, and S. Sripada. Berlin: Springer-Verlag

![](Figures/Chapter%2013/temporal%20data.png)
...
![](Figures/Chapter%2013/nation-stock%20temporal.png)
...
an few minutes, in busy trading. If there is less interest in the volatility of
...
built-in functions for querying time-varying data. Such queries can
...
![](Figures/Chapter%2013/Iberia-grid.png)
