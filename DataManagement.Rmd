# Data Management: Databases and Organizations {.unnumbered}
...
<!-- You can purchase a hardcopy from [Prospect Press](https://www.prospectpressvt.com). -->


<!--chapter:end:001-frontmatter.Rmd-->
...
The first map is based on the Newell-Simon model[^section1-1] of the human
...
[^section1-1]: Newell, A., & Simon, H. A. (1972). Human problem solving.
...
>
> Thomas Watson, Jr., former chairman of IBM[^data-1]

[^data-1]: Quinn, J. B. (1994). Appraising intellectual assets. \*The
...
power of information organizations.[^information-1] The need to create a sustainable
...
[^information-1]: In mid 2017, the five most valuable public companies were Apple,
...
management systems and UPS's telematics project.[^information-2] These new systems
...
[^information-2]: Watson, R. T., Boudreau, M.-C., Li, S., & Levis, J. (2010).
...
+----+------------+------------+------------+------------+
| E  | Focus      | Period     | Technology | Networks   |
| ra |            |            |            |            |
+====+============+============+============+============+
...
|    | computer   |            |            |            |
+----+------------+------------+------------+------------+
| 2  | Take       | Mid-1970s  | H          | Spread of  |
|    | i          | ---        | ost/termin | private    |
|    | nformation | mid-1990s  | al         | networks   |
|    | work to    |            |            |            |
|    | the        |            | Cli        |            |
|    | employee   |            | ent/server |            |
+----+------------+------------+------------+------------+
| 3  | Take       | Mid-1990s  | Brow       | Public     |
|    | i          | ---        | ser/server | networks\  |
|    | nformation | present    |            | (Internet) |
|    | work to    |            |            |            |
|    | the        |            |            |            |
|    | customer   |            |            |            |
|    | and other  |            |            |            |
...
[^information-3]: *Daft, R. L., & Lengel, R. H. (1986). Organizational information
...
method.[^information-4] Scorekeeping information is usually fairly stable.

[^information-4]: Rockart, J.F. (1982). The changing role of the information systems
...
<!--chapter:end:102-information.Rmd-->
...
<!--chapter:end:203-singleentity.Rmd-->
...
| [stkcode]{.ul} | stkfirm             | stkprice | stkqty  | stkdiv | stkpe | natname        | exchrate |
|:---------------|:--------------------|:---------|:--------|:-------|:------|:---------------|:---------|
...
Notice that `natcode` appears in both the `stock` and `nation` tables.
In `nation`, `natcode` is the primary key; it is unique for each
instance of `nation.` In table `stock`, `natcode` is a **foreign key**
because it is the primary key of `nation`, the one end of the 1:m
relationship. The column `natcode` is a foreign key in `stock` because
it is a primary key in `nation`. A matched primary key--foreign key pair
...
and arrows for the two USA stocks. In the `stock` table, `natcode` is
...
`stock.natcode.` Thus, `nation.natcode` refers to the `natcode` column
in the table `nation.`
...
value of `natcode` in `stock`. This requirement, known as the
**referential integrity constraint**, maintains the accuracy of a
...
example, it means that for every value of `natcode` in `stock`, there is
a corresponding entry in `nation`. As a result, a primary key row must
be created before its corresponding foreign key row. In other words,
details for a `nation` must be added before any data about its listed
stocks are entered.
...
relationship? Because each instance of `stock` is associated with
exactly one instance of `nation`. The rule is that a `stock` must be
...
`stock`, and this implies a multivalued foreign key. The relational
...
`fk_has_nation`. The foreign key is the column `natcode` in STOCK, and
it references the primary key of `nation`, which is `natcode`.
...
foreign key in `stock`. In this case, the ON DELETE clause specifies
that it is not permissible (the meaning of RESTRICT) to delete a primary
key row in `nation` while a corresponding foreign key in `stock` exists.
...
for a query into a single row. Consider the tables `nation` and `stock`.
...
with the same value for `natcode`. For example, to calculate the value
of Georgia Peach, which has `natcode` = 'US', find the row in `nation`
that also has `natcode` = 'US'. In this case, the stock's value is
...
The columns `stkprice` and `stkdiv` record values in the country's
...
-   To avoid confusion because `natcode` is a column name in both stock
    and nation, it needs to be qualified. If `natcode` is not qualified,
    the system will reject the query because it cannot distinguish
    between the two columns titled `natcode`.

-   The new table has the `natcode` column replicated. Both are called
    `natcode`. The naming convention for the replicated column varies
    with the RDBMS. The columns, for example, should be labeled
    `stock.natcode` and `nation.natcode`.
...
join, you will see that `nation` and `exchrate` are often repeated
because the same values apply to many stocks. A more appropriate format
is shown in the following figure, an example of a **control break
report**.

![](Figures/Chapter%204/cbr.png)

A control break report recognizes that the values in a particular column
or columns seldom change. In this case, `natname` and `exchrate` are
...
column `natname` is known as a *control field*. Notice that there are
four groups of data, because `natname` has four different values.
...
*List stocks by nation, and for each nation show the number of stocks
...
In this example, stocks are grouped by both `natname` and `stkpe`. In
most cases, there is only one stock for each pair of `natname` and
`stkpe`; however, there are two situations (U.K. stocks with PEs of 10
...
qualified as `nation.natcode` and `natcode` in the outer query is
understood to be `stock.natcode`.
...
`stock` and `nation` one row at a time and executing the subquery each
time. The first row has `natcode` = 'AUS' so the subquery becomes
...
execution depends on receiving a value for a variable (`nation.natcode`
...
-   There is a one-to-one correspondence between the names in
    ...
    the view column named `value` contains the result of the arithmetic
    expression `stkprice`\*`stkqty`\*`exchrate`.
...
excludes `stkqty`. In this case, changes in stock prices could be
...
> when discussing the correlated subquery? \| *Find those stocks where
...
9.  Find stocks where the third or fourth letter in their name is an
    'm'.

10. An electricity supply company needs a database to record details of
...
| [lineno]{.ul} | lineqty | lineprice | [*saleno*]{.ul} | *itemno* |
|:--------------|--------:|----------:|----------------:|---------:|
| 1             |       1 |      4.50 |               1 |        2 |
| 1             |       1 |     25.00 |               2 |        6 |
| 2             |       1 |     20.00 |               2 |       16 |
...
| [itemno]{.ul} | itemname              | itemtype | itemcolor |
|:--------------|:----------------------|:---------|:----------|
...
<!--chapter:end:205-manytomany.Rmd-->
...
<!--chapter:end:207-datamodeling.Rmd-->
...
<!--chapter:end:207r-basic-structures.Rmd-->
...
Consider the following table. The primary key of `order` is a composite
of `itemno` and `customerid`. The problem is that `customer-credit` is a
fact about `customerid` (part of the composite key) rather than the full
key (`itemno+customerid`), or in other words, it is not fully
...
| [itemno]{.ul} | [customerid]{.ul} | quantity | customer-credit |
|:--------------|:------------------|:---------|:----------------|
| 12            | 57                | 25       | OK              |
| 34            | 679               | 3        | POOR            |
...
The problem in the following table is that `exchange rate` is a fact
about `nation`, a nonkey field. In the language of functional
dependency, `exchange rate` is not fully functionally dependent on
`stockcode`, the primary key.

The functional dependencies are `stockcode` → `nation` →
`exchange rate`. In other words, `exchange rate` is transitively
dependent on STOCK, since `exchange rate` is dependent on `nation`, and
`nation` is dependent on `stockcode`. The fundamental problem becomes
very apparent when you try to add a new nation to the `stock` table.
...
MG from the `stock` table, details of the USA exchange rate are lost.
...
| [stockcode]{.ul} | nation | exchange rate |
|:-----------------|:-------|:--------------|
| MG               | USA    | 0.67          |
| IR               | AUS    | 0.46          |
...
> ❓*Skill builder* You have been given a spreadsheet that contains
> details of invoices. The\
> column headers for the spreadsheet are *date*, *invoice number*,
> *invoice amount*, *invoice tax*, *invoice total*, *cust number*,
> *custname*, *cust street*, *cust city*, *cust state*, *cust postal
> code*, *cust nation*, *product code*, *product price*, *product
> quantity*, *salesrep number*, *salesrep first name*, *salesrep last
> name*, *salesrep district*, *district name*, and *district size*.
...
| [client]{.ul} | [probtype]{.ul} | consultant |
|:--------------|:----------------|:-----------|
...
determinant, `consultant`→`probtype`, but is not a candidate key. In
terms of the phrasing used earlier, the problem is that part of the key
column is a fact about a nonkey column. The precise definition is *a
...
| [studentid]{.ul} | [sport]{.ul} | [subject]{.ul} | ... |
|:-----------------|:-------------|:---------------|:----|
| 50               | Football     | English        | ... |
| 50               | Football     | Music          | ... |
| 50               | Tennis       | Botany         | ... |
...
| [consultid]{.ul} | [firmid]{.ul} | [skilldesc]{.ul} |
|:-----------------|:--------------|:-----------------|
| Tan              | IBM           | Database         |
| Tan              | Apple         | Data comm        |
...
data about the firms a consultant advises; `advise` describes the skills
a consultant has; and `require` stores data about the types of skills
each firm requires. To help understand this problem, examine the three
tables below.

*Relational table contract*

| [consultid]{.ul} | [firmid]{.ul} |
|:-----------------|:--------------|
| Gonzales         | Apple         |
| Gonzales         | IBM           |
| Gonzales         | NEC           |
...
Consider joining `contract` and `advise`, the result of which we call
`could advise` because it lists skills the consultant could provide if
...
The join of `could advise` with `require` gives details of a firm's
skill needs that a consultant can provide. The table `can advise` is
constructed by directly joining `contract`, `advise`, and `require`.
Because we can construct `can advise` from three other tables, the data
are in 5NF.
...
5.  A telephone company offers a 10 percent discount to any customer who
    phones another person who is also a customer of the company. To be
    eligible for the discount, the pairing of the two phone numbers must
    be registered with the telephone company. Furthermore, for billing
    purposes, the company records both phone numbers, start time, end
...
<!--chapter:end:208-normalization.Rmd-->
...
<!--chapter:end:209-relational-model.Rmd-->
...
> ❓**Skill builder**  A national cinema chain has commissioned you to design a  
...
> ❓**Skill builder** Write SQL to answer each of the four preceding queries using the exped 
...
> ❓**Skill builder** Using MySQL's ROLLUP capability, report the sales by location and 
...
<!--chapter:end:318-clustercomputing.Rmd-->
...
[^dashboards-1]: <http://articles.latimes.com/2013/apr/18/news/la-paloma-picasso-discusses-new-olive-leaf-collection-for-tiffany-co-20130418>
...
*A dashboard
\<[[http://www.ecoresearch.net/climate/]{.ul}](http://www.ecoresearch.net/climate/)\>*
...
to learn Shiny. Some [[examples]{.ul}](http://www.apple.com) of
...
[[quantmod]{.ul}](https://cran.r-project.org/web/packages/quantmod/index.html)
...
![](Figures/Chapter 19/dashboard-2.png)
...
![](Figures/Chapter 19/dashboard-3.png)

> ❓*Skill builder* Add three more boxes (e.g., high price) to the dashboard just created.
...
![](Figures/Chapter 19/dashboard-4.png)
...
![](Figures/Chapter 19/dashboard-5.png)
...
![](Figures/Chapter 19/dashboard-6.png)
...
![](Figures/Chapter 19/dashboard-7.png)
...
![](Figures/Chapter 19/dashboard-8.png)
...
> ❓*Skill builder* Using ClassicModels, build a dashboard to report total orders by value and number for a given year and month.
...
3. Extend the previous dashboard. If the temperature is about 30C
...
<!--chapter:end:319-dashboards.Rmd-->
...
2.  The **file manager** converts this request into the address of the
...
3.  The **disk manager** determines the physical location of the page,
    issues the retrieval instructions, and passes the page to the file
    manager.

4.  The file manager extracts the requested record from the page and
...
![](Figures/Chapter%2020/disk-manager.png)
...
![](Figures/Chapter%2020/file-manager.png)
...
![](Figures/Chapter%204/nation-stock.png)
...
examined to determine the value of `itemtype`. For instance, if 20
...
small file containing just the value of `itemtype` for each record and
...
The `itemtype` index is a file. It contains 10,000 records and two
fields. There is one record in the index for each record in `item`. The
first columns contains a value of `itemtype` and the second contains a
pointer, an address, to the matching record of item. Notice that the
index is in `itemtype` sequence. Storing the index in a particular order
is another means of reducing disk accesses, as we will see shortly. The
index is quite small. One byte is required for `itemtype` and four bytes
...
accesses of `item`, as there are 2,000 items of type E. Creating an
...
Multiple indexes can be created for a file. The `item` file could have
an index defined on `itemcolor` or any other field. Multiple indexes may
...
The preceding query can be solved by using the indexes for `itemtype`
and `itemcolor`.
...
compressed by storing `itemno` for the second record on each page and
that page's address.
...
index is scanned to find the first value for `itemno` that is greater
...
Suppose that the query, *Find all stocks of country X*, is a frequent
...
In this example, we have two files: `nation` and `stock`. Records in
`nation` are connected by pointers (e.g., the horizontal arrow between
Australia and USA). The pointers are used to maintain the nation file in
logical sequence (by nation, in this case). Each record in `nation` is
linked to its `stock` records by a forward-pointing chain. The `nation`
record for Australia points to the first `stock` record (Indooroopilly
...
record in this `stock` chain points to the `nation` record to which the
...
(The parent in this case is `nation` and the child is `stock`.) Although
...
circumvent this is to build an index or hashing scheme for `stock` so
...
+-----------+---------+---------+--------------+
| i temcode | color   | code    | Disk address |
|           |         |         |              |
|           | char(8) | char(1) |              |
+===========+=========+=========+==============+
| 1001      | Blue    | N       | d1           |
...
a row is inserted into or deleted from the `nation` or `stock` tables.
...
Over USD 20 billion is spent annually on magnetic storage devices.[^datastruc-1] A
...
[^datastruc-1]: [\[[http://www.gartner.com/it/page.jsp?id=2149415]{.ul}\](http://www.terry.uga.edu/people/rwatson/)](http://www.gartner.com/it/page.jsp?id=2149415%5D%7B.underline%7D%5D(http://www.terry.uga.edu/people/rwatson/))
...
as little as twice as costly by 2021.[^datastruc-2] SSD is rapidly becoming the
standard for laptops.

[^datastruc-2]: [https://www.theregister.co.uk/2017/05/22/ssd\\\_price\\\_premium\\\_over\\\_disk\\\_falling/](https://www.theregister.co.uk/2017/05/22/ssd_price_premium_over_disk_falling/)
...
used in SSDs.[^datastruc-3] The introduction of tablets, such as the iPad, and
smartphones is exacerbating this problem.

[^datastruc-3]: <http://www.pcworld.com/businesscenter/article/217883/seagate_solidstate_disks_are_doomed_at_least_for_now.html>
...
> ❓*Skill builder* An IS department in a major public university records
...
+---------+---------+---------+---------+---------+---------+---------+
| D evice | A ccess | Vo lu   | Volat   | Cost    | Relia b | Legal   |
|         | speed   | me      | ility   | per me  | ility   | st a    |
|         |         |         |         | g abyte |         | nding   |
+:========+:========+:========+:========+:========+:========+:========+
| Solid   | \       | \*      | \       | \*      | \       | \*      |
| state   | \*\*\*  |         | \       |         | \*\*\*  |         |
...
3.  Describe the two types of delay that can occur prior to reading a
    record from a disk. What can be done to reduce these delays?

4.  What is clustering? What is the difference between intrafile and
...
8.  Write the SQL to create an index on the column natcode in the nation
    table.

9.  A Paris insurance firm keeps paper records of all policies and
...
10. A video producer has asked for your advice on a data storage device.
    She has specified that she must be able to record video at 5 to 7
    Mbytes per second. What would you recommend and why?

11. A German consumer research company collects scanning data from
...
12. A magazine subscription service has a Web site for customers to
...
13. A firm offers a satellite-based digital radio service to the
    ...
    a.  Assuming the average size of a digital audio file is 5 Mbytes
...
15. What is the data deluge? What are the implications for data
    management?

16. According to Wikipedia, Apple has more than 28 million songs in the
    iTunes store.[^datastruc-4] What storage technology might be a good choice for
    this library?

[^datastruc-4]: <http://en.wikipedia.org/wiki/ITunes_Store>

<!--chapter:end:420-datastrucandstor.Rmd-->
...
<!--chapter:end:421-architectures.Rmd-->
...
database, such as SQLite.[^administration-2]

[^administration-2]: <http://www.sqlite.org>
...
[^administration-3]: For more on information architecture, see Smith, H. A., Watson, R.
...
> ❓*Skill builder*A small nonprofit organization has asked for your help
...
a wide range of systems. The Transaction Processing Council (TPC)[^administration-4] is
the IS profession's Consumer Union. TPC has established benchmarks for a
variety of business situations.

[^administration-4]: <http://www.tpc.org>
...
+-------------------+------------------+-------------+---------------+
| Database          | Database         | Data        | Client        |
| development phase | developer        | ad          |               |
|                   |                  | ministrator |               |
+:==================+:=================+:============+:==============+
...
+-------------------+------------------+-------------+---------------+
| Database design   | Does             | Consults\   | Validates     |
|                   |                  | Data        | data models   |
|                   |                  | integrity   |               |
+-------------------+------------------+-------------+---------------+
| Database testing  | System and       | Consults\   | Testing       |
|                   | client testing   | Data        |               |
|                   |                  | integrity   |               |
+-------------------+------------------+-------------+---------------+
| Database          | System-related   | Consults\   | Client        |
| implementation    | activities       | Data        | activities    |
|                   |                  | integrity   |               |
+-------------------+------------------+-------------+---------------+
| Database usage    | Consults         | Data        | Uses          |
|                   |                  | integrity\  |               |
|                   |                  | monitoring  |               |
+-------------------+------------------+-------------+---------------+
| Database          | Does             | Change      | Provides      |
| evolution         |                  | control     | additional\   |
|                   |                  |             | requirements  |
+-------------------+------------------+-------------+---------------+
...
> ❓*Skill builder* Complete the data integration lab exercise described
...
<!--chapter:end:424-administration.Rmd-->
