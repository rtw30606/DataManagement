...
When designing the relational model, Codd provided commands for processing multiple records at a time. His intention was to increase the productivity of programmers by moving beyond the record-at-a-time processing that is found in most programming languages. Consequently, the relational model supports set processing (multiple records-at-a-time), which is most frequently implemented as **Structured Query Language (SQL).**[^single-1]

[^single-1]: Officially pronounced as "S-Q-L," but often also pronounced as "sequel".
...
MySQL Workbench is a professional quality, open source, cross-platform tool for data modeling and SQL querying.[^single-5] In this text, you will also learn some of the features of Workbench that support data modeling and using SQL. You will find it helpful to complete the MySQL Workbench tutorial on creating a data model[^single-6] prior to further reading, as we will assume you have such proficiency.

[^single-5]: <http://wb.mysql.com/>

[^single-6]: <http://dev.mysql.com/doc/workbench/en/wb-getting-started-tutorial-creating-a-model.html>
...
When specifying columns in Workbench you must also indicate the datatype. We opt to turn off the display of a column's datatype[^single-7] in a model to maintain focus on the entity.

[^single-7]: Preferences \> Diagram \> Show Column Types
...
MySQL Workbench offers a spreadsheet interface for entering data, as explained in the tutorial on adding data to a database.[^single-8]

[^single-8]: <http://dev.mysql.com/doc/workbench/en/wb-getting-started-tutorial-adding-data.html>
