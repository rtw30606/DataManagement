...
The exact same nation name and exchange rate  pair occurs 10 times
for stocks listed in the United Kingdom. This redundancy presents problems when we want to insert, delete, or update data. These problems,
...
the one nation. If exchange rate is stored in a nation table, however, only one change is necessary, there is no redundancy, and there is no danger of inconsistent exchange rates.
...
asterisking a primary key, is a useful reminder.
...
**referential integrity constraint**, helps maintain the accuracy of a
...
> record details of  campus buildings. A building can have many
...
tables,[^onetomany-1] we use lowercase table names and abbreviated
column names.
...
The HAVING clause in a GROUP BY is like the WHERE clause in a SELECT. It restricts the number of groups reported, whereas WHERE restricts the
...
more about regular expressions, see [regexlib.com](http://regexlib.com),
...
-   The six names enclosed in parentheses are the column names for the
    view.
...
> when discussing the correlated subquery? 
>
> *Find those stocks where
