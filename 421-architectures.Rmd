...
On a recent transatlantic flight, Sophie, the Personnel and PR manager,
had been seated next to a very charming young man who had tried to
impress her with his knowledge of computing. He worked for a large
systems consulting firm, and his speech was sprinkled with words like
"clouding computing," "XML," "server," "client," and "software as a
service." In return, Sophie responded with some "ums," "aahs," and an
occasional "that's interesting." Of course, this was before the third
glass of champagne. After that, he was more intent on finding out how
long Sophie would be in New York and what restaurants and shows piqued
her curiosity.
...
| ture |                                                               |
+:=====+:==============================================================+
| Two- | Processing is split between client and server, which also     |
| tier | runs the DBMS.                                                |
+------+---------------------------------------------------------------+
| Th   | Client does presentation, processing is done by the server,   |
| ree- | and the DBMS is on a separate server.                         |
| tier |                                                               |
+------+---------------------------------------------------------------+
| N-   | Client does presentation. Processing and DBMS can be spread   |
| tier | across multiple servers. This is a distributed resources      |
|      | environment.                                                  |
+------+---------------------------------------------------------------+
...
grid.[^1] This means that some cloud server farms are located where
...
[^1]: The further electrons are moved, the more energy is lost. As much
...
strategic implications.[^2] We start by looking at the levels or layers
of clouds.

[^2]: This section is based on Iyer, B., & Henderson, J. C. (2010).
...
+:=========+:=========================================+:===============+
| Infras   | A virtual server over which the          | Amazon Elastic |
| tructure | developer has complete control           | Compute Cloud  |
|          |                                          | (EC2)          |
+----------+------------------------------------------+----------------+
| Platform | A developer can build a new application  | Salesforce.com |
| as a     | with the provided tools and programming  |                |
| service  | language                                 |                |
+----------+------------------------------------------+----------------+
| App      | Access to cloud applications             | Google docs    |
| lication |                                          |                |
+----------+------------------------------------------+----------------+
| Colla    | A special case of an application cloud   | Facebook       |
| boration |                                          |                |
+----------+------------------------------------------+----------------+
...
Ownership is another way of looking at cloud offerings.[^3]

[^3]: Mell, P., & Grance, T. (2009). The NIST definition of cloud
