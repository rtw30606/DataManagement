...
*A relation is in first normal form if and only if all columns are single-valued.* 
In other words, 1NF states that all occurrences of a row
...
a component of the primary key. This can also be stated as *a relation is in second normal form if and only if it is in first normal form, and all nonkey columns are dependent on the key*.
...
![](Figures/Chapter 8/2NF.png)

### Third normal form {.unnumbered}

Third normal form is violated when a nonkey column is a fact about
another nonkey column. Alternatively, *a relation is in third normal form if and only if it is in second normal form and has no transitive dependencies.*
...
> ❓*Skill builder* You have been given a spreadsheet that contains details of invoices. The   
> column headers for the spreadsheet are *date*, *invoice number*,
> *invoice amount*, *invoice tax*, *invoice total*, *cust number*, *custname*,
> *cust street*, *cust city*, *cust state*, *cust postal code*,
...
is a fact about a nonkey column. The precise definition is *a relation is in Boyce-Codd normal form if and only if every determinant is a candidate key*.
...
solution is to convert multivalued dependencies to functional dependencies. 
More formally, *a relation is in fourth normal form if it is in Boyce-Codd normal form and all multivalued dependencies on the relation are functional dependencies.* A data model sorts out this
problem, and note that the correct relational mapping requires five tables.

*Resolving fourth normal form violation*

![](Figures/Chapter 8/4NF.png)
...
The preceding table is not in 5NF *if there is a rule of the following form*: 
If a consultant has a certain skill (e.g., database) and has a
...
Consider joining `contract` and `advise`, the result of which we call `could advise` 
because it lists skills the consultant could provide if the firm
...
Now we can give 5NF a more precise definition: 
*A relation is in fifth normal form if and only if every join dependency of the relation is a consequence of the candidate keys of the relation.*
...
DK/NF is easily stated. 
*A relation is in domain-key normal form if and only if every constraint on the relation is a logical consequence of the domain constraints and the key constraints that apply to the relation*.
...
![](Figures/Chapter 8/nation-stock-idefx.png)
...
| Constraint                           | IDEF1X                               |
| Data model                           | Inversion entry                      |
| Determinant                          | Many-to-many attribute relationship  |
| Domain                               | Multidetermination                   |
| Domain-key normal form (DK/NF)       | Multivalued dependency               |
| Entity                               | Normalization                        |
| Entity-relationship (E-R) model      | One-to-many attribute relationship   |
| Fifth normal form (5NF)              | One-to-one attribute relationship    |
| First normal form (1NF)              | Relationship                         |
| Fourth normal form (1NF)             | Second normal form (2NF)             |
| Full functional dependency           | Third normal form (3NF)              |
...
3. How do E-R and IDEF1X differ from the data modeling method of this
    text?
